...
namespace {
  ...
  struct TilingPass : public mlir::PassWrapper<TilingPass, OperationPass<func::FuncOp>> {
    ...
    void runOnOperation() override {
      ...
      func.walk([&](AddOp op) {
        ...
        SmallVector<OpFoldResult> offsets = {builder.getIndexAttr(START_OFFSET), builder.getIndexAttr(START_OFFSET)};
        SmallVector<OpFoldResult> sizes = {builder.getIndexAttr(TILE_SIZE), builder.getIndexAttr(TILE_SIZE)};
        ...
        FailureOr<TilingResult> tilingResult = op.getTiledImplementation(builder, offsets, sizes);
        ...
        for (int64_t i = START_OFFSET; i < resultTensorShape[0]; i += TILE_SIZE) {
          for (int64_t j = START_OFFSET; j < resultTensorShape[1]; j += TILE_SIZE) {
            SmallVector<OpFoldResult> currentOffset = {
                    builder.getIndexAttr(i),
                    builder.getIndexAttr(j)
            };

            resultTensor = builder.create<tensor::InsertSliceOp>(
                    op.getLoc(), *currentTile, resultTensor,
                    currentOffset, resultTensorSize, strides);
            ++currentTile;
          }
        }
